...
for full_file_name in os.listdir(input_folder):
    ...
    print('---------------------------------------------')
    print('-- PROCESSANDO O ARQUIVO: ' + full_file_name)
    print('---------------------------------------------')
    ...
    df_excel.to_excel(os.path.join(output_folder, file_name + '.xlsx'), index=False)
    print('OK')
    ...
    text_file = open(os.path.join(output_folder, file_name + '.html'), "w", newline='', encoding='utf-8')
...
# you will probably notice that by default, pandas writes the line index as the first column in the spreadsheet
# if you don't want that, you can switch it off by adding the parameter index=False
df_colunada.to_excel(os.path.join(output_folder, 'Geral_UtilizadaAtualmente' + '.xlsx'), index=False)
print('OK')
