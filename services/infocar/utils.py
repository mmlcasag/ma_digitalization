...
def to_string(original_input):
    str_input = str(original_input).strip()
...
def to_date(original_input):
    ...
    if len(str_input) == 10 and (str_input[4:5] == "-" or str_input[4:5] == "/"):
        return datetime.strptime(
            f"{str_input[8:10]}/{str_input[5:7]}/{str_input[0:4]}", "%d/%m/%Y"
        )
    if len(str_input) == 10 and (str_input[2:3] == "-" or str_input[2:3] == "/"):
        return datetime.strptime(
...
def from_float_to_string(original_float, decimal_places=None):
    if decimal_places is None:
        return str(original_float).replace(",", "").replace(".", ",")
    else:
        return (
            str(f"%.{decimal_places}f" % round(original_float, decimal_places))
            .replace(",", "")
            .replace(".", ",")
        )
...
def from_datetime_to_string(original_datetime):
    if original_datetime is None:
        ...
        return original_datetime.strftime("%d/%m/%Y %H:%M:%S")
